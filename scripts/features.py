...
def features(seqID, sequence):
    ...
    parameters = parameters + str(round(percent["A"] * 100, 4)) + "," + str(round(percent["C"] * 100, 4)) + "," + str(
        round(percent["D"] * 100, 4)) + "," + str(round(percent["E"] * 100, 4)) + "," + str(
        round(percent["F"] * 100, 4)) \
                 + "," + str(round(percent["G"] * 100, 4)) + "," + str(round(percent["H"] * 100, 4)) + "," + str(
        round(percent["I"] * 100, 4)) + "," + str(round(percent["K"] * 100, 4)) + "," + str(
        round(percent["L"] * 100, 4)) + "," + str(round(percent["M"] * 100, 4)) \
                 + "," + str(round(percent["N"] * 100, 4)) + "," + str(round(percent["P"] * 100, 4)) + "," + str(
        round(percent["Q"] * 100, 4)) + "," + str(round(percent["R"] * 100, 4)) + "," + str(
        round(percent["S"] * 100, 4)) + "," + str(round(percent["T"] * 100, 4)) \
                 + "," + str(round(percent["V"] * 100, 4)) + "," + str(round(percent["W"] * 100, 4)) + "," + str(
        round(percent["Y"] * 100, 4)) + ","
    ...
    parameters = parameters + str(round(X.secondary_structure_fraction()[0], 4)) + "," + str(
        round(X.secondary_structure_fraction()[1], 4)) + "," + str(round(X.secondary_structure_fraction()[2], 4)) + ","
    ...
    small = count["A"] + count["C"] + count["D"] + count["G"] + count["N"] + count["P"] + count["S"] + count["T"] + \
            count["V"]
    ...
    polar = count["D"] + count["E"] + count["H"] + count["K"] + count["N"] + count["Q"] + count["R"] + count["S"] + \
            count["T"]
    polar_per = round(((float(polar) / len(seq)) * 100), 3)
    parameters = parameters + str(polar_per) + ","

    nonPolar = count["A"] + count["C"] + count["F"] + count["G"] + count["I"] + count["L"] + count["M"] + count["P"] + \
               count["V"] + count["W"] + count["Y"]
