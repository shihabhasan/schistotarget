...
import sqlite3
import hashlib
sys.path.append('/home/shihab/schistotarget/scripts')
...
Multi-layer Perceptron Prediction,Multi-layer Perceptron Decision Score,Multi-layer Perceptron Probability\n")
    ...
    for record in records:
        ...
        data=Immuno.objects.filter(sequence=hash_sequence)
        if len(data)==0:
   ...
   #---------------------WORKING WITH SCIKIT-LEARN------------
    if parameter!="":
        ...
        train_positive_file="immuno_positive.csv"
        train_negative_file="immuno_negative.csv"
        ...
        for seqID in seqID_list:
            ...
            p=Immuno(sequence=hashlib.md5(str(fasta_rec[seqID].seq)).hexdigest(), prediction=result_dict[seqID], features=str(param[j]), access=0, time=datetime.now())    
            p.save()
            j=j+1
    ...
    return (prediction_lines, header, lines, x1, x2, x3, fh, fd)
