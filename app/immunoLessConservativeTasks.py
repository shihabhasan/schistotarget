...
from app.models import ImmunoLessConservative
...
#---------------------IMMUNOREACTIVITY LESS CONSERVATIVE------------------

def run_immuno_less_conservative(filename, input_email, feature_mode, task_id):
    parameter=""
    test_features=""
    test_para=""
    seqID_list=[]
    ...
    Quadratic Discriminant Analysis Prediction,Quadratic Discriminant Analysis Decision Score,Quadratic Discriminant Analysis Probability,\
    ...
    for record in records:
        ...
        data=ImmunoLessConservative.objects.filter(sequence=hash_sequence)
        if len(data)==0:
   ...
   #---------------------WORKING WITH SCIKIT-LEARN------------
    if parameter!="":
        ...
        train_positive_file="immuno_positive_less_conservative.csv"
        train_negative_file="immuno_negative_less_conservative.csv"
        ...
        names = [gbm, svm_rbf, svm_linear, decision_tree, random_forest, ada_boost, \
                 naive_bayes, lda, qda, ridge_regression, sgd, perceptron, \
        ...
        for seqID in seqID_list:
            result_dict[seqID]=gbm[seqID]+"\t"+svm_rbf[seqID]+"\t"+svm_linear[seqID]+"\t"+decision_tree[seqID]+"\t"+\
                                random_forest[seqID]+"\t"+ada_boost[seqID]+"\t"+naive_bayes[seqID]+"\t"+lda[seqID]+"\t"+qda[seqID]+"\t"+\
            ...
            p=ImmunoLessConservative(sequence=hashlib.md5(str(fasta_rec[seqID].seq)).hexdigest(), prediction=result_dict[seqID], features=str(param[j]), access=0, time=datetime.now())
            p.save()
            j=j+1
    ...
    for line in lines:
        ...
        if prediction_count>8:
            if len(str(fasta_record[l[0]].seq))<20:
                ...
                predicted=l[0]+',Non-Immunoreactive Protein,'+str(17-prediction_count)+" / 17"
        else:
            if len(str(fasta_record[l[0]].seq))<20:
                predicted = l[0] + ',Too short sequence,Unable to predict'
            else:
                predicted=l[0]+',Immunoreactive Protein,'+str(17-prediction_count)+" / 17"
